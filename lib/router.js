...
function createRouter(resolvePath) {
	...
	router.get('/:script/events', function (req, res, next) {
		...
		fs.stat(output_file+'/' + req.params.script + '_speed.txt', function(err, stat) {
			if(err == null) {
				fs.readFile(output_file+'/' + req.params.script + '_speed.txt', 'utf-8' ,function(err, buf) {
					var contents = buf.toString();
					var arr = contents.split(";");
					onEvent({speed: arr[0], ospeed: arr[1], unique: 0, type: "speed"})
				});
			}
		});
	...
	router.post('/:script/events', bodyParser.json(), function (req, res, next) {
		...
		if(event.type == "speed") {
			fs.writeFile(output_file+'/' + req.params.script + '_speed.txt', event.speed+";"+event.ospeed, function(err, data){
			});
		} else if(event.type == "play") {
			var buf = fs.readFileSync(output_file+'/' + req.params.script + '_speed.txt', 'utf-8')

			var contents = buf.toString();
			var arr = contents.split(";");
			req.body = {speed: arr[1], ospeed: arr[1], unique: 1, type: "speed"}
		}
